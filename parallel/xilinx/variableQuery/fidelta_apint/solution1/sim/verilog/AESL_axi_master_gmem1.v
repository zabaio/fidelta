...
//------------------------Parameter----------------------
`define TV_IN_gmem1 "../tv/cdatafile/c.accel_in_circle_accel_in_circle.autotvin_gmem1.dat"
`define TV_OUT_gmem1 "../tv/rtldatafile/rtl.accel_in_circle_accel_in_circle.autotvout_gmem1.dat"
...
parameter gmem1_mem_depth = 32'd 4;
...
output [64 - 1 : 0] TRAN_gmem1_indata_V;
input ready;
input done;
...
reg [gmem1_DATA_BITWIDTH - 1 : 0] gmem1_mem_3 [0: gmem1_mem_depth - 1]; 
...
assign    TRAN_gmem1_indata_V = indata_V;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/users/alberto.giusti/Desktop/backup/variableQuery/fidelta_apint/solution1/sim/verilog/AESL_axi_master_gmem1.v: Write request address %d exceed AXI master gmem1 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/users/alberto.giusti/Desktop/backup/variableQuery/fidelta_apint/solution1/sim/verilog/AESL_axi_master_gmem1.v: Read request address %d exceed AXI master gmem1 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  reg [1047 : 0] token; 
  reg [1047 : 0] token_tmp; 
  //reg [gmem1_C_DATA_BITWIDTH - 1 : 0] token_tmp; 
  reg [gmem1_DATA_BITWIDTH - 1 : 0] mem_tmp; 
  integer i; 
  transaction_num = 0;
  gmem1_bitwidth = gmem1_C_DATA_BITWIDTH; 
  count_seperate_factor_by_bitwidth (gmem1_bitwidth , factor);
  fp = $fopen(`TV_IN_gmem1,"r"); 
