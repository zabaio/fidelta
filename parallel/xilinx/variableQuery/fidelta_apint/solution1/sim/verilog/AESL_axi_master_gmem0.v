...
module AESL_axi_master_gmem0 (
    ...
    TRAN_gmem0_BID,
    TRAN_gmem0_BUSER,
    TRAN_gmem0_instate_V,
    ready,
    done
    );

//------------------------Parameter----------------------
`define TV_IN_gmem0 "../tv/cdatafile/c.accel_in_circle_accel_in_circle.autotvin_gmem0.dat"
`define TV_OUT_gmem0 "../tv/rtldatafile/rtl.accel_in_circle_accel_in_circle.autotvout_gmem0.dat"
 parameter gmem0_ADDR_BITWIDTH = 32'd 64;
 parameter gmem0_AWUSER_BITWIDTH = 32'd 1;
 parameter gmem0_DATA_BITWIDTH = 32'd 256;
...
parameter gmem0_C_DATA_BITWIDTH = 32'd 256;
parameter gmem0_mem_depth = 32'd 4;
...
output [64 - 1 : 0] TRAN_gmem0_instate_V;
input ready;
input done;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [64 - 1 : 0] instate_V = 0;
reg [gmem0_DATA_BITWIDTH - 1 : 0] gmem0_mem_0 [0: gmem0_mem_depth - 1]; 
reg [gmem0_DATA_BITWIDTH - 1 : 0] gmem0_mem_1 [0: gmem0_mem_depth - 1]; 
reg [gmem0_DATA_BITWIDTH - 1 : 0] gmem0_mem_2 [0: gmem0_mem_depth - 1]; 
reg [gmem0_DATA_BITWIDTH - 1 : 0] gmem0_mem_3 [0: gmem0_mem_depth - 1]; 
...
assign    TRAN_gmem0_instate_V = instate_V;
...
initial begin : initialize_offset
  ...
  instate_V <= 0 * DATA_byte_num;
end

initial begin : initialize_gmem0_mem
  integer i;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem0_mem_depth) begin
                $display ("/home/users/alberto.giusti/Desktop/backup/variableQuery/fidelta_apint/solution1/sim/verilog/AESL_axi_master_gmem0.v: Write request address %d exceed AXI master gmem0 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem0_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem0_mem_depth) begin
                $display ("/home/users/alberto.giusti/Desktop/backup/variableQuery/fidelta_apint/solution1/sim/verilog/AESL_axi_master_gmem0.v: Read request address %d exceed AXI master gmem0 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem0_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  reg [535 : 0] token; 
  reg [535 : 0] token_tmp; 
  //reg [gmem0_C_DATA_BITWIDTH - 1 : 0] token_tmp; 
  ...
  while(1) begin 
      ...
      for(i = 0; i < (gmem0_mem_depth - gmem0_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  ...
                  3 : tmp_data = gmem0_mem_3[i/factor];
      ...
      if (factor == 4) begin
          case(mem_page)
              ...
              3 : tmp_data = gmem0_mem_3[gmem0_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
      ...
      if (factor == 2) begin
          if ((gmem0_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : $fdisplay(fp,"0x%x",gmem0_mem_0[gmem0_mem_depth/factor][15:0]);
                  1 : $fdisplay(fp,"0x%x",gmem0_mem_1[gmem0_mem_depth/factor][15:0]);
                  2 : $fdisplay(fp,"0x%x",gmem0_mem_2[gmem0_mem_depth/factor][15:0]);
                  3 : $fdisplay(fp,"0x%x",gmem0_mem_3[gmem0_mem_depth/factor][15:0]);
                  default: $display("The page_num of write file is not valid!");
              endcase
          end
      end
