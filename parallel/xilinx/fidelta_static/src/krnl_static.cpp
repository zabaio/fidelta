...
void accel_in_circle(ap_uint<512> *indata, ap_uint<64> *instate, ap_uint<64> *outstate, int inquery){
	...
	#pragma HLS INTERFACE m_axi port=outstate offset=slave bundle=gmem2

	#pragma HLS INTERFACE s_axilite port=indata bundle=control
	#pragma HLS INTERFACE s_axilite port=instate bundle=control
	#pragma HLS INTERFACE s_axilite port=outstate bundle=control
	#pragma HLS INTERFACE s_axilite port=inquery bundle=control
	...
	mainloop: for (i = 0; i < query/2; i++){
		...
		memcpy(outstate + i, temp_state, sizeof(ap_int<64>));
	}
